...
youbot_dh_parameters = {'a':[-0.033, 0.155, 0.135, 0, 0],
                        'alpha': [np.pi/2, 0, 0, np.pi/2, 0],
                        'd' : [0.019, 0, 0, -0.002, 0.055],
                        'theta' : [0, np.pi/2, 0, np.pi/2, np.pi/2]}

# TODO: populate the values inside the youbot_joint_offsets dictionary with the ones you found in question 5c.
youbot_joint_offsets = [2.967, -1.1345, 2.548, -1.789, 2.9234]
...
name_link = ['arm5c_link_1', 'arm5c_link_2', 'arm5c_link_3', 'arm5c_link_4', 'arm5c_link_5']
...
def fkine_wrapper(joint_msg, br):
    ...
    for i in range(5):
        T_current = forward_kinematics(youbot_dh_parameters, list(joint_msg.position), i+1)
        transform.header.stamp = rospy.Time.now()
        transform.header.frame_id = 'base_link'
        transform.child_frame_id = name_link[i]

        transform.transform.translation.x = T_current[0, 3]
        transform.transform.translation.y = T_current[1, 3]

        transform.transform.translation.z = T_current[2, 3]
        transform.transform.rotation = rotmat2q(T_current)
